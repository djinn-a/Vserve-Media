...
const getL2RTranslateX = (
  ...
  // Phase 1: Entering (from -100 up to 0) -> Left to Right
  if (progress <= peakProgress) {
    const enterRatio = clamp((progress - startProgress) / INTERVAL, 0, 1);
    return -100 + (enterRatio * 100);
  }
  
  // Phase 2: Exiting (from 0 to 100 or -100) -> Continue Right (100) except i=4 exits Left (-100)
...
const getR2LTranslateX = (
  ...
  // Phase 1: Entering (from 100 down to 0) -> Right to Left
  if (progress <= peakProgress) {
    const enterRatio = clamp((progress - startProgress) / INTERVAL, 0, 1);
    return 100 - (enterRatio * 100);
  }
  
  // Phase 2: Exiting (from 0 down to -100) -> Continue Left (-100)
  const exitRatio = clamp((progress - peakProgress) / INTERVAL, 0, 1);
  return -100 * exitRatio;
};

export function ScrollImageSequenceReverse() {
  ...
  useEffect(() => {
    ...
    const renderTransforms = (scrollY: number) => {
      ...
      panelsRef.current.forEach((panel, index) => {
        ...
        // Reversed Logic:
        // i <= 4 now uses Left-To-Right
        // i > 4 now uses Right-To-Left
  ...
  return (
    <section 
      ref={containerRef} 
      id="mobile-scroll-sequence-reverse" 
...
export default ScrollImageSequenceReverse;
